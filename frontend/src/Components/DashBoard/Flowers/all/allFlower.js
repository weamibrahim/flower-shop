...
class AllFlower extends Component {
  ...
  handleDelete = (id) => {
    ...
    fetch(`https://flower-shop-roan.vercel.app/api/flower/delete/${id}`, {
      method: "DELETE",
      headers: {
      ...
      .then((data) => {
        ...
        const updatedflowers = this.state.flowers.filter(
          (flower) => flower._id !== id
        );
                      ...
                      <tr key={flower.id}>
                        ...
                        <td>
                          <button
                            ...
                            onClick={() => this.handleDelete(flower._id)}
                          >
                            <MdDelete className="fs-2" />
                          </button>
                          <button className="btn btn-success mx-1">
                            <NavLink
                              to={`updateflower/${flower._id}`}
                              className="text-decoration-none text-white"
                            >
                              <LiaEdit className="fs-2" />
                            </NavLink>{" "}
                          </button>
                        </td>
                      </tr>
